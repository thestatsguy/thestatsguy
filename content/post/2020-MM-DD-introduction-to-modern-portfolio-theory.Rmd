...
**_Terminology_** ![alt text](https://github.com/adam-p/markdown-here/raw/master/src/common/images/icon48.png "Logo Title Text 1")
...
In other words, the portfolio is the linear combination of the various assets under consideration. Of course, the question that becomes: how to choose $w = (w_1, ..., w_p)^T$ such that expected returns are maximised while minimising risk?
...
$$\sigma_N^2 = \sum_i\sum_jw_iw_j\sigma_{ij} = w_T\Sigma w$$

## Starting with two assets

Again, the objective of treating portfolio selection as a statistical problem is to select individual $w_i$ such that $ER_N$ is large and $\sigma_N^2$ is small. We can start solving this problem by first considering $p = 2$, that is, choosing the weights $w_i$ between 2 assets only. Inevitably, we would have

$$
\begin{array}{c}
ER_N = \sum_iw_iER_i = w_1ER_1 + w_2ER_2 \\\
