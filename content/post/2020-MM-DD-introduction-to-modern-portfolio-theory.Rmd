...
Suppose there are $p$ assets with returns
...
\begin{array}{c}
r_i = ER_i, \sigma_i = \sigma_{R_i}, cov(R) = \Sigma = (\sigma_{ij})_{1 \le i,j \le p}
\end{array}
$$
...
\end{array}
$$
where $w_i$ is the weight of the $i$th asset in the portfolio, such that
...
$$
Of course, the questions that becomes: how to choose $w = (w_1, ..., w_p)^T$ such that expected returns are maximised while minimising risk?
