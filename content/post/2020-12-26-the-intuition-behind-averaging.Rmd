...
![A peculiar example](images/2020-12-26-the-intuition-behind-averaging-01.png)
...
* __Approach 4 - what can reduce our error?__: firstly, what does "belong to the same group" mean? The most intuitive way of extrapolating from that is that we can at least guess that the mystery number should be close to the other numbers of the circle - i.e. we have no reason to think that it's smaller than the smallest number, or larger than the largest number, and have some intuition to guess that the mystery number is somewhere within the smallest and the largest number. A reasonable intuition.
...
```
Step 1. Hide 2 and treat 2 as ?, use the rest to guess, compare guess with actual value (2).
